    ...
    if uploaded_file is not None:
        ...
        if st.button("분석 시작"):
            with st.spinner("분석 중..."):
                ...
                # 기획자 아이디어 도출
                idea_prompt = f"""다음은 특정 사업의 RFP 요약 내용입니다:

                {rfp_summary}

                이 사업 내용을 따르면서 효과적으로 운영하기 위한 기획자의 인사이트와 창의성이 담긴 아이디어를 300자 내외로 작성해주세요. 
                특정 프로그램이 아닌 과업의 전반적 방향성을 설정하는 아이디어여야 합니다. 
                사업의 목적을 달성하면서도 혁신적이고 효과적인 접근 방식을 제안해주세요."""

                planner_idea = generate_content(idea_prompt)
                st.subheader("기획자 아이디어")
                st.write(planner_idea)

                # 커뮤니케이션 전략 생성 (슬로건 포함)
                comm_prompt = f"""다음은 RFP의 요약 내용과 기획자의 아이디어입니다:
                ...
                위의 내용을 바탕으로 커뮤니케이션 전략과 슬로건을 생성해주세요. 다음 구조화된 형식을 따라 작성해주세요:

                슬로건: [사업의 핵심을 간결하고 임팩트 있게 표현한 슬로건]
                ...
                # 성공 전략 생성
                success_prompt = f"""다음은 RFP의 요약 내용과 기획자의 아이디어입니다:
                ...
                위의 내용을 바탕으로 성공 전략을 생성해주세요. 다음 구조화된 형식을 따라 작성해주세요:
